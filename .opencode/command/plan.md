...
`<itr root>` is set dynamically based on configuration. It contains a BACKLOG folder and a TASKS folder, among other project files. The BACKLOG folder contains yaml files that describe upcoming features and tasks that need to be implemented.
 
Plans are created in `<itr root>/TASKS/<backlog-id>/` and should be named `<task-id>-plan.md`. Each plan should be a detailed document that outlines the steps needed to implement the feature described in the corresponding BACKLOG item, along with any necessary context, dependencies, and acceptance criteria.
...
Create a plan for the next task to work on. The task id is provided as `$ARGUMENTS` (e.g. `backlog-take`). The task file is found at `<itr root>/TASKS/<backlog-id>/<task-id>-task.yaml`.
...
3. Read the task file at `<itr root>/TASKS/<backlog-id>/<task-id>-task.yaml` to identify the source backlog id and repo scope. The task id is provided via `$ARGUMENTS`.
4. Read the backlog item at `<itr root>/BACKLOG/items/<backlog-id>.yaml` for the full feature description, acceptance criteria, and dependencies.
...
7. Save the plan in a new file in the `<itr root>/TASKS/<backlog-id>/` directory with a descriptive name (e.g., `<task-id>-plan.md`).
...
The output should be a markdown file in the `<itr root>/TASKS/<backlog-id>/` directory with a detailed plan for implementing the next task, following the structure outlined in step 5. The file should be well-organized and clearly written to guide the implementation process.
