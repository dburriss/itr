...
module PortfolioFormatter =
    ...
    let formatProfileList (format: OutputFormat) (profiles: ProfileRow list) : unit =
            ...
            let items =
                ...
                |> List.map (fun p ->
                    ...
                    sprintf "  { \"name\": \"%s\", \"isDefault\": %s, \"gitName\": \"%s\", \"gitEmail\": \"%s\", \"productCount\": %d }"
                        p.Name isDefaultStr p.GitName p.GitEmail p.ProductCount)
                |> String.concat ",\n"
            printfn "["
        ...
        | Table ->
            ...
            |> List.iter (fun p ->
                let marker = if p.IsDefault then "*" else ""
                table.AddRow(marker, p.Name, p.GitName, p.GitEmail, string p.ProductCount) |> ignore)
    ...
    let formatProductList (format: OutputFormat) (rows: ProductRow list) : unit =
        ...
        | Text ->
            rows
            |> List.iter (fun r ->
                printfn "%s\t%d\t%s" r.Id r.RepoCount r.CoordRoot)
        | Table ->
            let table = Table()
            ...
            rows
            |> List.iter (fun r ->
                table.AddRow(r.Id, string r.RepoCount, r.CoordRoot) |> ignore)
    ...
    let formatProductInfo (format: OutputFormat) (data: ProductInfoData) : unit =
        ...
        | Json ->
            ...
            printfn "  \"docs\": {"
            if not data.Docs.IsEmpty then printfn "%s" docsJson
            printfn "  },"
            printfn "  \"repos\": {"
            if not data.Repos.IsEmpty then printfn "%s" reposJson
            printfn "  },"
        ...
        | Text ->
            ...
            data.Docs |> List.iter (fun (key, absPath) -> printfn "docs\t%s\t%s" key absPath)
            data.Repos |> List.iter (fun (key, absPath, _) -> printfn "repos\t%s\t%s" key absPath)
            printfn "coordMode\t%s" data.CoordMode
            printfn "coordRepo\t%s" data.CoordRepo
            printfn "coordPath\t%s" data.CoordPath
        | Table ->
            ...
            data.Docs |> List.iter (fun (key, absPath) -> table.AddRow("docs", key, absPath) |> ignore)
            data.Repos |> List.iter (fun (key, absPath, _) -> table.AddRow("repos", key, absPath) |> ignore)
