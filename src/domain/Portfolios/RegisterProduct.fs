...
type Input =
    { Path: string; Profile: string option }
                    ...
                    if System.String.IsNullOrWhiteSpace(input.Path) then
                        ...
                        if not (fs.DirectoryExists resolvedPath) then
                            ...
                            | Ok definition ->
                                ...
                                | Ok existingPairs ->
                                    let isDuplicate =
                                        existingPairs |> List.exists (fun (_, def) -> ProductId.value def.Id = newId)
                                    ...
                                    if isDuplicate then
                                        ...
                                        let updatedProfile =
                                            { profile with
                                                Products = profile.Products @ [ newRef ] }

                                        let updatedProfiles =
                                            portfolio.Profiles |> Map.add profile.Name updatedProfile
